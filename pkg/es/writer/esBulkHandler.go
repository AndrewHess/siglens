...
func ProcessBulkRequest(ctx *fasthttp.RequestCtx, myid uint64, useIngestHook bool) {
	...
	if hook := hooks.GlobalHooks.OverrideIngestRequestHook; hook != nil {
		alreadyHandled, primary, rid = hook(ctx, myid, grpc.INGEST_FUNC_ES_BULK, useIngestHook)
		if alreadyHandled {
			return
		}
	}

	processedCount, response, err := HandleBulkBody(ctx.PostBody(), ctx, myid, rid, primary, useIngestHook)
	if err != nil {
...
func HandleBulkBody(postBody []byte, ctx *fasthttp.RequestCtx, myid uint64, rid uint64, primary bool, useIngestHook bool) (int, map[string]interface{}, error) {
		...
		case INDEX, CREATE:
			...
			if numBytes < segment.MAX_RECORD_SIZE {
				...
				if strings.Contains(indexName, ".kibana") {
					...
					err := ProcessIndexRequest(rawJson, tsNow, indexName, uint64(numBytes), false, localIndexMap, myid, rid, primary)
					if err != nil {
						success = false
					}
				}
...
func ProcessIndexRequest(rawJson []byte, tsNow uint64, indexNameIn string,
	bytesReceived uint64, flush bool, localIndexMap map[string]string, myid uint64, rid uint64, primary bool) error {
