...
func ProcessRedTracesIngest() {
	...
	for service, spanCnt := range serviceToSpanCnt {
		...
		// Ingest red metrics
		err = writer.ProcessIndexRequest(jsonData, now, indexName, lenJsonData, shouldFlush, localIndexMap, orgId, 0 /*TODO*/, true /*TODO*/)
		if err != nil {
...
func writeDependencyMatrix(dependencyMatrix map[string]map[string]int) {
	...
	// Ingest
	err = writer.ProcessIndexRequest(dependencyMatrixJSON, now, indexName, lenJsonData, shouldFlush, localIndexMap, orgId, 0 /*TODO*/, true /*TODO*/)
	if err != nil {
